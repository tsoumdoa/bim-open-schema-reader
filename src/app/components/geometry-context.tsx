...
interface FilteredGeometryResult {
...
function rowsToInstanceData(rows: (string | number)[][]): InstanceData[] {
	return rows.map((row) => ({
...
export function GeometryProvider(props: {
	...
	const getFilteredScene = (
		...
		const totalCount = new Set(
			cache.instanceData.map((inst) => inst.entity_index)
		).size;
